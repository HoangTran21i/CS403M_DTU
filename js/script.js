...
// JavaScript để cập nhật size và số lượng khi người dùng chọn
document.querySelectorAll('input[name="product_size"]').forEach((sizeInput) => {
    sizeInput.addEventListener('change', function () {
        document.getElementById('selected-size').textContent = this.value; // Ghi lại size
    });
});

const quantityInput = document.getElementById('quantity_input');
quantityInput.addEventListener('input', function () {
    document.getElementById('selected-quantity').textContent = this.value; // Ghi lại số lượng
});